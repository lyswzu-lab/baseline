...
from utilities.dataset import nihchest,mimic,chexpert
opj = os.path.join
...
def get_dataset(args):
    # [!! 已修改 !!] 添加了 MIMIC, CHEXPERT, NIH
    data_dict = {'MS-COCO': COCO2014, 'VOC2007': VOC2007, 'MIMIC': mimic, 'CHEXPERT': chexpert, 'NIH': nihchest}
    ...
    # [!! 新增 !!] 处理 MIMIC
        ...
        # (根据 image_240a60.png, 假设 'mimic' 文件夹在 args.data_root 下)
        data_dir = opj(args.data_root) # root 路径就是 mimic 文件夹
        test_set = data_dict[args.data_set](data_dir, phase='test',
                                            transform=test_transfm)
        train_set = data_dict[args.data_set](data_dir, phase='train',
                                             transform=train_transfm)
    ...
    # [!! 新增 !!] 处理 CHEXPERT
        ...
        # (根据 image_240ab9.png, 假设 'chexpert' 文件夹在 args.data_root 下)
        data_dir = opj(args.data_root) # root 路径就是 chexpert 文件夹
        test_set = data_dict[args.data_set](data_dir, phase='test',
                                            transform=test_transfm)
        train_set = data_dict[args.data_set](data_dir, phase='train',
                                             transform=train_transfm)
    ...
    # [!! 新增 !!] 处理 NIH
    elif args.data_set in ('NIH'):
        # (根据 image_240a1d.png, 假设 'nih-chest-xray' 文件夹在 args.data_root 下)
        data_dir = opj(args.data_root) # root 路径就是 nih-chest-xray 文件夹
        test_set = data_dict[args.data_set](data_dir, phase='test',
                                            transform=test_transfm)
        train_set = data_dict[args.data_set](data_dir, phase='train',
                                             transform=train_transfm)
